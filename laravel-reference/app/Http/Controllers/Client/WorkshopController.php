...
class WorkshopController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            $company_workshops = Workshop::query()
                ->with('workshop_case')
            ...
            $data = [
                'workshops' => $company_workshops,
            ];
        } catch (ModelNotFoundException) {
            abort(404);
        }

        foreach ($company_workshops as $workshop) {
            if ($workshop->workshop_case) {
                $workshop->workshop_case->full_time = $this->readable_time($workshop->workshop_case->full_time);
            }
        }
        ...
        return view('client.workshops')->with($data);
    }
}
