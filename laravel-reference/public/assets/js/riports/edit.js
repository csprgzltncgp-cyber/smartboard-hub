...
function select_change() {
    $('select[name="cities"]').change(function () {
        ...
        const name = $(this).find('option[value="' + value + '"]').html();
        const html = '<p class="value cities">' + name + ': <input type="number" name="cities[' + value + ']" value="0"></p>';
        $(this).before(html);
        $(this).val(0);
    })
}
...
function save_on_click_outside() {
    $('.container').on('focusout', 'input[type="number"]', function () {
        const element = $(this);
        if (element.closest('.value').find('button').length !== 0) {
            edit_input_value_count(element.attr('name'));
        }
    });
}

function edit_input_value_count(selector) {
    const element = $(`input[name="${selector}"]`);
    const value = parseInt(element.val());
    const input = element.attr('name');
    const name = input.split('[')[0];
    const key = input.match(/\[(\d+)\]/) ? input.match(/\[(\d+)\]/)[1] : 0;
    const country = input.replace(name, '').replace('[' + key + ']', '').match(/\[(\d+)\]/) ? input.replace(name, '').replace('[' + key + ']', '').match(/\[(\d+)\]/)[1] : null;
    ...
    $.ajax({
        ...
        type: 'POST',
        url: '/ajax/edit-riport-value-count',
        data: {
            id: riport_id,
            name: name,
            key: key,
