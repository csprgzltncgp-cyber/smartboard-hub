...
@section('content')
    <div class="row">
        <div class="col-12 mb-5 p-0">
            @if(url()->current() == route(auth()->user()->type . '.eap-online.translation.assessment.view'))
                {{Breadcrumbs::render('eap-online.translate-assessment')}}
                <h1>Assessment</h1>
            @else
                {{Breadcrumbs::render('eap-online.translate-well-being')}}
                <h1>Well-Being</h1>
            @endif
        </div>
        <div>
                    ...
                    <form method="post" class="w-100">
                        <div class="my-4">
                            <div class="d-flex flex-column">
                                ...
                                <div class="d-flex flex-column">
                                    @foreach($question->eap_assessment_answers as $answer)
                                        <div class="d-flex justify-content-between align-items-center line mb-3"
                    ...
                    <form method="post" class="w-100">
                        ...
                        <div class="d-none" id="result-{{$result->id}}-translations">
                            ...
                            <div class="col-12 mt-1 mb-4">
                                <button class="btn-radius btn-max-width float-right d-flex justify-content-center align-items-center" type="submit">
                                    <img class="mr-1" src="{{asset('assets/img/save.svg')}}" style="height: 20px; width: 20px" alt="">
                                    {{__('common.save')}}
                                </button>
                            </div>
            ...
            <div class="row d-flex flex-column col-12 mt-5">
                @foreach($types as $type)
                    <div class="d-flex justify-content-between align-items-center line mb-3"
                         onclick="toggleTranslationSection('type-{{$type->id}}-translations', this)"
                    >
                        <p class="m-0 mr-3">{{__('eap-online.articles.category')}} {{$loop->index + 1}}</p>
                        <div class="d-flex align-items-center">
                            <div class='d-flex flex-wrap'>
                                @foreach($languages as $language)
                                    @if(!empty($type->translations()->where('language_id', $language->id)->first()))
                                        <div style="background-color:rgb(145,183,82);"
                                            class="px-2 text-white mr-3 mb-2">
                                            {{$language->code}}
                                        </div>
                    ...
                    <form method="post" class="w-100">
                        @csrf
                        <div class="d-none" id="type-{{$type->id}}-translations">
                            @foreach($languages as $language)
                                <div class="row translation">
                                    <div class="col-1 text-center" style="padding-top:15px;">
                                        {{$language->code}}
                                    ...
                                    <div class="col-8 pl-0">
                                        <textarea name="types[{{$type->id}}][{{$language->id}}]"
                                                  placeholder="{{__('eap-online.system.translation')}}">{{$type->translations()->where('language_id', $language->id)->first()->value ?? ''}}</textarea>
                                    </div>
                                </div>
                            @endforeach
                            <div class="col-12 mt-1 mb-4">
