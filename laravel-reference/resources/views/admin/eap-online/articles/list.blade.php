    ...
    <script>
        $(function () {
            ...
            $(document).on('click', '.pagination a', function (e) {
                getArticles($('#search').val(), $(this).attr('href').split('page=')[1])
                e.preventDefault();
            });
        });
        ...
        function getArticles(needle = '', page) {
            let url;
            if (page) {
                url = `/ajax/get-articles?page=${page}`
            } else {
            ...
            $.ajax({
                ...
                data: {
                    needle,
                    translation: '{{ (Route::is('admin.eap-online.articles.translate.list') || Route::is('production_translating_admin.eap-online.articles.translate.list')) }}'
                },
        ...
        <div class="col-12  mb-5 d-flex flex-column align-items-start">
            <div class="w-100 flex flex-col align-items-center col-12 p-0">
                @if(Route::is('admin.eap-online.articles.list'))
                    <div>
                        {{ Breadcrumbs::render('eap-online.articles') }}
                        <h1>EAP online - {{ __('eap-online.articles.menu') }}</h1>
                        <a href="{{route('admin.eap-online.articles.new')}}">{{__('eap-online.articles.new')}}</a>
                    </div>
                @endif
                @if((Route::is('admin.eap-online.articles.translate.list') || Route::is('production_translating_admin.eap-online.articles.translate.list')))
                    {{ Breadcrumbs::render('eap-online.translate-articles') }}
                    <h1>EAP online - {{__('eap-online.articles.translate')}}</h1>
                @endif
                <div class="d-flex flex-row mb-4 mt-3">
                    ...
                    <button class="ml-3 btn-radius" style="--btn-height:auto; --btn-margin-bottom: 0px;" id="reset_search" onClick="resetSearch()">
