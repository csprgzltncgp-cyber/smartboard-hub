    ...
    <script>
        $(function () {
            ...
            $(document).ajaxStop(function () {
                $('#videos_holder').removeClass('d-none');
                $('#spinner').addClass('d-none');
            });

            $(document).on('click', '.pagination a', function (e) {
                getVideos($('#search').val(), $(this).attr('href').split('page=')[1])
                e.preventDefault();
            });
        ...
        function getVideos(needle = '', page) {
            let url;
            if (page) {
                url = `/ajax/get-videos?page=${page}`
            } else {
                url = `/ajax/get-videos`
            }

            $('#videos_holder').addClass('d-none');
            ...
            $.ajax({
                ...
                data: {
                    needle,
                    translation: '{{Route::is('admin.eap-online.videos.translate.list')}}'
                },
                success: function (data) {
                    $('#videos_holder').html(data);
                },
            });
        }
    </script>
    ...
    <div class="row">
        <div class="col-12  mb-5 d-flex flex-column align-items-start">
            <div class="w-100 flex flex-col align-items-center col-12 p-0">
                @if(Route::is('admin.eap-online.videos.list'))
                    <div>
                        {{ Breadcrumbs::render('eap-online.videos') }}
                        <h1>EAP online - {{__('eap-online.videos.menu')}}</h1>
                        <a href="{{route('admin.eap-online.videos.new_view')}}">{{__('eap-online.videos.add')}}</a>
                    </div>
                @endif
                @if(Route::is('admin.eap-online.videos.translate.list'))
                    {{ Breadcrumbs::render('eap-online.translate-videos') }}
                    <h1>EAP online - {{__('eap-online.videos.translate')}}</h1>
                @endif
                <div class="d-flex mb-4 mt-3">
                    @if(Route::is('admin.eap-online.videos.list'))
                        <a href="{{route('admin.eap-online.filter.view', ['model' => 'videos'])}}" id="filter"
        ...
        <div class="col-12" id="videos_holder"></div>
