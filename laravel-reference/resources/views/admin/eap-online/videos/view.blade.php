...
@section('content')
    <div class="row">
        <div class="col-12">
            {{ Breadcrumbs::render('eap-online.videos.edit', $video) }}
            <h1>{{__('eap-online.videos.edit')}}</h1>
        </div>
        <div class="col-12">
            <form id="saveForm" action="{{route('admin.eap-online.videos.edit', ['id' => $video->id])}}" method="post"
                  class="mb-5"
                  style="max-width: 100%"
                ...
                <div class="row d-flex flex-column col-12">
                    <h1 class="mb-3">{{__('eap-online.articles.appearance')}} <span id="visibility_alt"
                                class="h5 {{ ($video->eap_visibility->burnout_page || $video->eap_visibility->domestic_violence_page) ? 'd-none' : '' }}">{{__('eap-online.videos.visibility_alt')}}</span></h1>
                    <select name="visibility">
                        <option value="null">{{__('eap-online.articles.none')}}</option>
                        <option value="theme_of_the_month"
                                @if($video->eap_visibility->theme_of_the_month) selected @endif>{{__('eap-online.articles.theme_of_the_month')}}</option>
                        <option value="home_page"
                                @if($video->eap_visibility->home_page) selected @endif>{{__('eap-online.articles.home_page')}}</option>
                        <option value="burnout_page"
                                @if($video->eap_visibility->burnout_page) selected @endif>{{__('eap-online.articles.burnout_page')}}</option>
                        <option value="domestic_violence_page"
                                @if($video->eap_visibility->domestic_violence_page) selected @endif>{{__('eap-online.articles.domestic_violence_page')}}</option>
                    </select>
                    <div id="apperance-more"
                         class="@if(!($video->eap_visibility->theme_of_the_month || $video->eap_visibility->home_page)) d-none @endif">
                        <div id="date_picker"
                             class="@if($video->eap_visibility->theme_of_the_month)d-flex @else d-none @endif flex-column mb-4 ml-n2"
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                <span class="pt-1"
                                      id="visibility_date">
                                    @if(empty($video->eap_visibility->to_date))
                                        <span class="pt-1"
                                              id="visibility_date">{{__('eap-online.articles.select_date')}}</span>
                                    @else
                                        <span class="pt-1"
                                              id="visibility_date">{{$video->eap_visibility->from_date}} - {{$video->eap_visibility->to_date}}</span>
                                    @endif
                                </span>
                            </div>
                        </div>
                ...
                <div id="categorize" class="row {{ ($video->eap_visibility->burnout_page || $video->eap_visibility->domestic_violence_page) ? 'd-none' : 'd-flex' }} flex-column col-12">
                                    ...
                                    <li class="flex-grow-1">
                                        <label class="container pb-2"
                                            ...
                                            <input type="radio" name="categories[]{{$loop->index}}"
                                                   value="{{$category->id}}"
                                                   @if($video->hasCategory($category->id))
                                                   checked="checked"
                                                    @endif>
                                            <span class="checkmark"></span>
                                        </label>
                                        @if(count($category->childs))
                                            @include('components.eap-online.category-line ',['childs' => $category->childs, 'level' => 1, 'resource' => $video, 'type' => $loop->index])
                                        @endif
                                    </li>
                                @endforeach
                            </ul>
    ...
    <path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" />
                        ...
                        <div class="col-12 d-none list-el pl-0" data-category="3">
                            <ul class="border-0 d-inline-flex w-100 mb-2 pb-0 pt-0 mt-4 pl-0"
                                style="color: black !important; list-style: none;">
                                @foreach(\App\Models\EapOnline\EapCategory::whereNull('parent_id')->where('type', 'all-videos')->with('childs')->get() as $category)
                                    <li class="flex-grow-1">
                                        <label class="container pb-2"
                                               id="customer-satisfaction-not-possible">{{$category->name}}
                                            <input type="checkbox" name="categories[]{{$loop->index}}"
                ...
                <div id="lesson" class="row {{ $video->eap_visibility->burnout_page ? 'd-flex' : 'd-none' }} flex-column col-12">
                    <h1 class="mb-3">{{__('eap-online.articles.lesson')}}</h1>
                    <select name="lesson">
                        <option value="1"
                                @if(optional($video->lesson)->value == '1') selected @endif>{{__('eap-online.articles.lesson_1')}}</option>
                        <option value="2"
                                @if(optional($video->lesson)->value == '2') selected @endif>{{__('eap-online.articles.lesson_2')}}</option>
                        <option value="3"
                                @if(optional($video->lesson)->value == '3') selected @endif>{{__('eap-online.articles.lesson_3')}}</option>
                        <option value="4"
                                @if(optional($video->lesson)->value == '4') selected @endif>{{__('eap-online.articles.lesson_4')}}</option>
                        <option value="5"
                                @if(optional($video->lesson)->value == '5') selected @endif>{{__('eap-online.articles.lesson_5')}}</option>
                        <option value="6"
                                @if(optional($video->lesson)->value == '6') selected @endif>{{__('eap-online.articles.lesson_6')}}</option>
                        <option value="7"
                                @if(optional($video->lesson)->value == '7') selected @endif>{{__('eap-online.articles.lesson_7')}}</option>
                    </select>
                </div>

                <div id="chapter" class="row {{ $video->eap_visibility->domestic_violence_page ? 'd-flex' : 'd-none' }} flex-column col-12">
                    <h1 class="mb-3">{{__('eap-online.articles.chapter')}}</h1>
                    <select name="chapter">
                        <option value="1"
                                @if(optional($video->chapter)->value == '1') selected @endif>{{__('eap-online.articles.chapter_1')}}</option>
                        <option value="2"
                                @if(optional($video->chapter)->value == '2') selected @endif>{{__('eap-online.articles.chapter_2')}}</option>
                        <option value="3"
                                @if(optional($video->chapter)->value == '3') selected @endif>{{__('eap-online.articles.chapter_3')}}</option>
                        <option value="4"
                                @if(optional($video->chapter)->value == '4') selected @endif>{{__('eap-online.articles.chapter_4')}}</option>
                        <option value="5"
                                @if(optional($video->chapter)->value == '5') selected @endif>{{__('eap-online.articles.chapter_5')}}</option>
                        <option value="6"
                                @if(optional($video->chapter)->value == '6') selected @endif>{{__('eap-online.articles.chapter_6')}}</option>
                    </select>
                </div>
                    ...
                    <div class="d-flex flex-column col-6 row">
                        ...
                        <div class="d-flex align-items-center">
                            <svg onclick="deleteUploadedFile()"
                                 class="@if(! $video->eap_video_attachment()->where('language_id', $video->language)->exists())d-none @endif mr-1 ml-n1"
                            ...
                            <svg onclick="triggerFileUpload()"
                                 ...
                                 class="ml-n1 mr-1 @if($video->eap_video_attachment()->where('language_id', $video->language)->exists())d-none @endif ml-n1"
                                 xmlns="http://www.w3.org/2000/svg"
                                 style="@if($errors->has('attachment')) color:#db0b20 @else color: rgb(89, 198, 198); @endif  height: 20px; width: 20px; cursor: pointer"
                                 fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <span id="uploaded-file-name">
                                @if($video->eap_video_attachment()->where('language_id', $video->language)->exists())
                                    {{$video->eap_video_attachment()->where('language_id', $video->language)->first()->filename}}
                                @else
                        ...
                        <div class="d-flex align-items-center">
                            <div class="mr-5 w-100">
                                <input oninput="removeError('error-text', 'download-button-text')" class="mb-0"
                                       ...
                                       value="@if($video->eap_video_attachment()->where('language_id', $video->language)->exists()) {{$video->eap_video_attachment->where('language_id', $video->language)->first()->button_text}}@endif"
                                >
                            </div>
                        </div>
                    </div>
                ...
                <div class="row d-flex col-7">
                    <h1 id="link"
                        class="@if($errors->has('link')) error-text @endif">{{__('eap-online.videos.link')}}</h1>
                    <input oninput="removeError('error-text', 'link')" type="text" placeholder="link..." name="link"
                           value="@if($video->link) {{$video->link}} @endif">
                </div>
                <div class="row d-flex col-7">
                    <h1 id="short_title"
                    ...
                    <input oninput="removeError('error-text', 'short_title')" type="text"
                           ...
                           value="@if($video->short_title) {{$video->short_title}}@endif">
                </div>
                <div class="row d-flex col-7">
                    <h1 id="long_title"
                    ...
                    <input oninput="removeError('error-text', 'long_title')" type="text"
                           ...
                           value="@if($video->long_title) {{$video->long_title}}@endif">
                </div>
                <div class="row d-flex col-7">
                    ...
                    <input oninput="removeError('error-text', 'description_first_line')" type="text"
                           ...
                           value="@if($video->description_first_line) {{$video->description_first_line}}@endif">
                </div>
                <div class="row d-flex col-7 ">
                    ...
                    <input oninput="removeError('error-text', 'description_second_line')" type="text"
                           ...
                           value="@if($video->description_second_line) {{$video->description_second_line}}@endif">
                </div>
                ...
                <input type="hidden" name="start_date"
                       value="{{!empty($video->eap_visibility->from_date) ? $video->eap_visibility->from_date : ''}}">
                <input type="hidden" name="end_date"
                       value="{{!empty($video->eap_visibility->to_date) ? $video->eap_visibility->to_date : ''}}">
            </form>
        </div>
    </div>

    <form method="post" id="deleteForm"
          action="{{route('admin.eap-online.videos.delete', ['id' => $video->id])}}">{{csrf_field()}}</form>
@endsection
