...
@section('extra_js')
    <script>
        $(function () {
            ...
            $('#search').on('input', function (e) {
                if (e.target.value.length >= 2 || e.target.value.length == 0) {
                    getWebinars(e.target.value)
                }
            });

            $(document).ajaxStop(function () {
                $('#webinars_holder').removeClass('d-none');
                $('#spinner').addClass('d-none');
            });

            $(document).on('click', '.pagination a', function (e) {
                getWebinars($('#search').val(), $(this).attr('href').split('page=')[1])
                e.preventDefault();
            });
        ...
        function getWebinars(needle = '', page) {
            let url;
            if (page) {
                url = `/ajax/get-webinars?page=${page}`
            } else {
                url = `/ajax/get-webinars`
            }

            $('#webinars_holder').addClass('d-none');
            ...
            $.ajax({
                ...
                data: {
                    needle,
                    translation: '{{Route::is('admin.eap-online.webinars.translate.list')}}'
                },
                success: function (data) {
                    $('#webinars_holder').html(data);
                },
            });
        }
    </script>
    ...
    <div class="row">
        <div class="col-12  mb-5 d-flex flex-column align-items-start">
            <div class="w-100 flex flex-col align-items-center col-12 p-0">
                @if(Route::is('admin.eap-online.webinars.list'))
                    <div>
                        {{ Breadcrumbs::render('eap-online.webinars') }}
                        <h1>EAP online - {{__('eap-online.webinars.menu')}}</h1>
                        <a href="{{route('admin.eap-online.webinars.new_view')}}">{{__('eap-online.webinars.add')}}</a>
                    </div>
                @endif
                @if(Route::is('admin.eap-online.webinars.translate.list'))
                    {{ Breadcrumbs::render('eap-online.translate-webinars') }}
                    <h1>EAP online - {{__('eap-online.webinars.translate')}}</h1>
                @endif
                <div class="d-flex mb-4 mt-3">
                    @if(Route::is('admin.eap-online.webinars.list'))
                        <a href="{{route('admin.eap-online.filter.view', ['model' => 'webinars'])}}" id="filter"
                    ...
                    <button class="flex-grow-1 btn-radius btn-max-width d-flex flex-row justify-content-center align-items-center" id="reset_search"
        ...
        <div class="col-12" id="webinars_holder"></div>
