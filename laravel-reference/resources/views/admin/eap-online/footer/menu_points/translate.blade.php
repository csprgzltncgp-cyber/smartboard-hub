...
@section('content')
    <div class="row">
        <div class="col-12 mb-5 p-0">
            {{ Breadcrumbs::render('eap-online.translate-footer-menu') }}
            <h1>EAP online - {{__('eap-online.footer.menu_points.translate')}}</h1>
        </div>
        <form method="post" class="row w-100" action="{{route('admin.eap-online.footer.menu.translate.store')}}">
            <div class="col-12">
                {{csrf_field()}}
                    ...
                    <div class="col-12 input">
                        <div class="row">
                            <div class="col-12 pl-3 d-flex justify-content-between align-items-center mb-3 line"
                                 onclick="toggleTranslationSection('menu_point-{{$menu_point->id}}-translations', this)"
                            >
                                <p class="m-0 mr-3">{{ucfirst($menu_point->firstTranslation->value)}}</p>
                                <div class="d-flex align-items-center">
                                    <div class='d-flex flex-wrap'>
                                        @foreach($languages as $language)
                                            @if(!empty($menu_point->get_translation($language->id)))
                                ...
                                <input type="hidden" name="menu_points[{{$loop->index}}][id]" value="{{$menu_point->id}}"/>
                            </div>
                        </div>
                        <div class="d-none" id="menu_point-{{$menu_point->id}}-translations">
                            @foreach($languages as $language)
                                <div class="row translation">
                                    ...
                                    <div class="col-8 pl-0">
                                        <textarea name="menu_points[{{$loop->parent->index}}][text][{{$language->id}}]"
                                                  placeholder="{{__('eap-online.system.translation')}}">{{$menu_point->get_translation($language->id)->value ?? ''}}</textarea>
                                    </div>
                                </div>
                            @endforeach
                            <div class="col-12  mt-1 mb-4">
                                <button class="button btn-max-width btn-radius d-flex justify-content-center align-items-center" type="submit">
