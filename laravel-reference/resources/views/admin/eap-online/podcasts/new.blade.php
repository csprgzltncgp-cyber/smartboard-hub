...
@section('extra_js')
    ...
    <script type="text/javascript">
        ...
        const category_trans = "{{__('eap-online.podcasts.all_podcasts')}}";
        const short_title_trans = "{{__('eap-online.videos.short_title')}}";
        const long_title_trans = "{{__('eap-online.videos.long_title')}}";
        const link_trans = "{{__('eap-online.podcasts.link')}}";
        const required_trans = "{{__('eap-online.required')}}";
        let main_categories = @json(\App\Models\EapOnline\EapCategory::where('type', 'all-podcasts')->get()->pluck('id')->toArray())
    </script>
...
@section('content')
    <div class="row">
        <div class="col-12">
            {{ Breadcrumbs::render('eap-online.podcasts.create') }}
            <h1>{{__('eap-online.podcasts.add')}}</h1>
        </div>
        <div class="col-12">
            <form action="{{route('admin.eap-online.podcasts.new')}}" method="post" class="mb-5" style="max-width: 100%"
                ...
                <div class="row d-flex flex-column col-12">
                    ...
                    <div class="col-3 row d-flex">
                        <button id="language-select-button"
                                class="float-left btn-radius @if($errors->has('language')) error @endif" type="button"
