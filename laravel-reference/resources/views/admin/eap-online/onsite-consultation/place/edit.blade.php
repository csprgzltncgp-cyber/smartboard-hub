...
@section('content')
    <div class="row">
        <div class="row col-12">
            <div class="col-12">
                {{ Breadcrumbs::render('eap-online.onsite-consultation.place.edit', $place) }}
                <h1>{{ __('eap-online.onsite_consultation.edit_place') }}</h1>

                <div>
                    <form method="POST" action="{{route('admin.eap-online.onsite-consultation.place.update')}}">
                        {{csrf_field()}}
                        <input type="hidden" name="onsite_consultation_place_id" value="{{$place->id}}">
                        <div class="mb-1 mt-3" style="border: 2px solid rgb(89,198,198) !important;">
                            <div class="d-flex flex-row align-items-center pl-2">
                                ...
                                <input type="text" name="name" value="{{$place->name}}" style="margin:0px!important; padding: 10px 0px 10px 0px !important; border:0px!important; color:black!important; background-color:transparent">                
                            </div>
                        </div>
                        <div class="mb-1 mt-3" style="border: 2px solid rgb(89,198,198) !important;">
                            <div class="d-flex flex-row align-items-center pl-2">
                                <span class="mr-1" style="color: rgb(89, 198, 198);">{{ __('eap-online.onsite_consultation.place_address') }}:</span>
                                <input type="text" name="address" value="{{$place->address}}" style="margin:0px!important; padding: 10px 0px 10px 0px !important; border:0px!important; color:black!important; background-color:transparent">               
                            </div>
                        </div>
                        <button type="submit"
