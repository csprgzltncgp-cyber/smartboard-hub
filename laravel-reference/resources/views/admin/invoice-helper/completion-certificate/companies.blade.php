...
@section('extra_js')
    <script>
        ...
        const months_container = document.getElementById('months_container');
         ...
         function downloadAllCompletionCertificates(date){
            ...
            $.ajax({
                ...
                url: '/admin/invoice-helper/completion-certificate/all/download/' + date,
                success: function (response) {
                    ...
                    a.download = 'completion-certificates.pdf'
        ...
        @foreach(collect($dates)->reverse() as $date)
            <div class="case-list-in col-12 group">
                ...
                <div class="caret-left float-right">
                    ...
                    <a onclick="downloadAllCompletionCertificates('{{$date}}')" class="print_all_text float-right" style="{{all_completion_certificates_printed_in_month($date) ? 'color:#a33095;' : 'color: #59c6c6;'}} margin-left:10px">
                        ...
                        @endif
                    <span>{{__('invoice-helper.completion-certificate.print-all')}}</span>
                    </a>
                </div>
            </div>
            @livewire('admin.invoice-helper.completion-certificate.month', ['date' => $date->format('Y-m-d')], key('date-' . $date->format('Y-m-d')))
        @endforeach
    </div>
