    ...
    <script>
        ...
        function getContent(needle = '') {
            $.ajax({
                ...
                type: 'GET',
                url: '/ajax/get-feedback-translations-lines',
                data: {
                    needle
                },
        ...
        function createRowHTML(line, index, data) {
            ...
            data.languages.forEach((language) => {
                if (language.code in line.text) {
                    html += `
                                         <div style="background-color:rgb(145,183,82);" class="px-2 text-white mr-3">
                                            ${language.code}
                                        </div>`;
                } else {
                    html += `
                                         <div style="background-color:rgb(219, 11, 32);" class="px-2 text-white mr-3">
                                            ${language.code}
                                        </div>`;
                }
            });
...
@section('content')
    <div class="row">
        <div class="col-12 mb-5 p-0">
            {{ Breadcrumbs::render('feedback.translations') }}
            <h1>{{__('myeap.system.translations')}}</h1>
            <div class="w-100 d-flex flex-row align-items-center col-12 pl-0 ml-0">
                <button class="btn-radius" onClick="addInput()" style="--btn-height: 44px; --btn-margin-bottom: 0px;">{{__('eap-online.system.new')}}</button>
                <input class="mb-0" style="height:50px;" type="text" id="search" placeholder="{{__('eap-online.system.search')}}">
                <button class="flex-grow-1 btn-radius" onClick="resetSearch()"
                style="--btn-height: 44px; --btn-margin-bottom: 0px; --btn-margin-left: var(--btn-margin-x)">
