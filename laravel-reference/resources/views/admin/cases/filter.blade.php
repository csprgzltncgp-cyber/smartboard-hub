...
@section('extra_js')
    ...
    <script>
        ...
        @if(session()->has('expert_and_date_interval_required'))
            Swal.fire(
                '{{__('common.filter_expert_and_consultation_date_required')}}!',
                '',
                'error'
            );
        @endif
...
@section('content')
    <div class="row">
        <div class="col-12">
            {{ Breadcrumbs::render('closed-cases.filter') }}
            ...
            <form method="post" class="row">
                ...
                @foreach($filters as $filter)
                    @php
                        $filterTranslation = $filter->translation ? $filter->translation->value : 'Nincs fordítás';
                    @endphp
                    <div class="filter-holder col-6">
                        <div class="filter">
                            <p>{{$filterTranslation}}</p>
                            <button type="button" class="filter-button">
                                <svg xmlns="http://www.w3.org/2000/svg" style="height: 30px" fill="none"
                            ...
                            <div class="options d-none">
                                @if($filter->type == 'date')
                                    <input type="text" name="filter[{{$filter->id}}][from][]" class="datepicker"
                                           placeholder="{{ __('common.from') }}" style="width:38%;margin-right:4%;"/>
                                    <input type="text" name="filter[{{$filter->id}}][to][]" class="datepicker"
                                           placeholder="{{ __('common.to') }}" style="width:38%;"/>
                                @elseif($filter->type == 'text')
                                    <input type="text" name="filter[{{$filter->id}}][value][]"
                                           placeholder="{{$filterTranslation}}">
                                @elseif($filter->type == 'integer')
                                    <input type="number" name="filter[{{$filter->id}}][value][]"
                                           placeholder="{{$filterTranslation}}">
                                ...
                                @elseif($filter->default_type == 'clients_language' )
                                    <select name="filter[{{$filter->id}}][value][]">
                                        <option value="-1">{{__('common.please-choose-one')}}</option>
                                        @foreach($languageSkills as $language)
                                        <option value="{{$language->id}}">{{$language->translation->value}}</option>
                ...
                <div class="filter-holder col-6">
                    <div class="filter">
                        ...
                        <div class="options d-none">
                            <input type="text" name="case_confirmed_at_from" class="datepicker"
                                           placeholder="{{ __('common.from') }}" style="width:38%;margin-right:4%;"/>
                            <input type="text" name="case_confirmed_at_to" class="datepicker"
                                    placeholder="{{ __('common.to') }}" style="width:38%;"/>
                        </div>
                    </div>
                </div>
                ...
                <div class="filter-holder col-6">
                    <div class="filter">
                        <p>{{__('common.filter_total_consultations')}}</p>
                        <button type="button" class="filter-button">
                            <svg xmlns="http://www.w3.org/2000/svg" style="height: 30px" fill="none" viewBox="0 0 24 24"
                                 stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="options d-none">
                            <select name="show_consultation_numbers_total">
                                <option value="-1">{{ __('common.please-choose') }}</option>
                                <option value="1">{{__('common.yes')}}</option>
                                <option value="0">{{__('common.no')}}</option>
                            </select>
