...
@section('content')
    <div class="row m-0 w-100">
        {{ Breadcrumbs::render('todo.filtered') }}
        <h1 class="col-12 pl-0">{{__('workshop.filter_result')}}</h1>
        <ul id="workshop-submenus" class="row ml-0 w-100">
            <li class="mr-0"><a class="col-12 ml-0 d-block add-new-workshop btn-radius" href="{{route(auth()->user()->type . '.todo.filter')}}">
            ...
            <li class="ml-0"><a class="col-12 pl-0 d-block add-new-workshop btn-radius" href="{{route(auth()->user()->type . '.dashboard')}}">
