@push('livewire_js')
    <script>
        $('#orderBySelectUpcomming').on('change', function(val) {
            var orderBy = $(this).val();
            @this.orderBy = orderBy;
        });

        $('#orderBySelectUpcomming option[value="{{implode(',', $this->orderBy)}}"]').attr("selected",true);
    </script>
@endpush
...
<div>
    @if($affiliateSearches->count())
        <div class="headline-container">
            <h1 class="headline-black" style="background:rgba(89, 198, 198, 0.2);">
                {{__('task.upcomming')}}: {{$affiliateSearches->total()}}
            </h1>

            <x-tasks.sort id="orderBySelectUpcomming"/>
        </div>

        <div class="card" style="background: rgba(89, 198, 198, 0.2);">
            ...
            @if($affiliateSearches->hasMorePages())
                <div class="d-flex justify-content-center" style="margin-bottom: -25px; margin-top:25px;">
                    ...
                    <button class="button-link" wire:click.prevent='loadAll'>
                        <svg xmlns="http://www.w3.org/2000/svg" class="mr-1" style="width: 20px; height: 20px" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 13l-7 7-7-7"></path>
                        </svg>
                        {{__('common.load-all')}}
                    </button>
                </div>
