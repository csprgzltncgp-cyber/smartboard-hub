            ...
            @foreach($companies as $company)
                <div class="invoice-list-holder" wire:key='company-{{$company->id}}'>
                    <div class="case-list-in col-12 group" wire:click='toggleOpenCompany({{$company->id}})'>
                        ...
                        <div class="float-right" style="{{$company->direct_invoices->first()->envelope->printed_at ? 'color:#a33095;' : 'color: #59c6c6;'}}">
                            @if($company->direct_invoices->first()->envelope->printed_at)
                           ...
                           <span wire:loading.delay.remove wire:click='download_envelope({{$company->id}}, "{{$date}}")'>{{__('invoice-helper.envelope.print')}}</span>
                        </div>
                    </div>
                </div>
            @endforeach
