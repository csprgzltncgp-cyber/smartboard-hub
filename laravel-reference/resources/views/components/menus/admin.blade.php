...
<li><a href="{{route('admin.submenu.digital')}}">{{__('common.digital')}}</a></li>
<li><a href="{{route('admin.submenu.settings')}}">{{__('common.submenu.settings')}}</a></li>
<li><a href="{{route('admin.submenu.outsources')}}">{{__('common.submenu.outsources')}}</a></li>
<li><a href="{{route('admin.submenu.riports')}}">{{__('common.submenu.riports')}}</a></li>
<li><a href="{{route('admin.submenu.invoices')}}">{{__('common.submenu.invoices')}}</a></li>
<li><a href="{{route('admin.notifications.list')}}">{{__('common.notification')}}</a></li>
<li><a href="{{route('admin.assets.menu')}}">{{__('common.assets')}}</a></li>
...
@if(auth()->user()->connected_account == null)
    <li class='d-flex justify-content-between align-items-center'>
        <a href="{{route('admin.dashboard')}}">TODO</a>
        <div>
            ...
            @if($unreadTaskComments)
                <span style="width: 36px; height: 36px; padding: 8px; text-align: center; background: #007bff; border-radius: 50%; float: right; color: white" class="unread_counter mr-2">
    ...
    <li class='d-flex justify-content-between align-items-center'>
        <a href="{{route('admin.affiliate_searches.index')}}">{{__('affiliate-search-workflow.menu')}}</a>
        <div>
            @if(!empty($newAffiliateSearchess) && $newAffiliateSearchess > 0)
                <span style="width: 36px; height: 36px; padding: 8px; text-align: center; background: rgb(145, 183, 82); border-radius: 50%; float: right; color: white"
                    class="unread_counter">{{$newAffiliateSearchess}}</span>
            @endif

            @if($unreadAffiliateSearchComments)
                <span style="width: 36px; height: 36px; padding: 8px; text-align: center; background: #007bff; border-radius: 50%; float: right; color: white"
