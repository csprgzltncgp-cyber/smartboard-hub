    ...
    <div class="row mt-5">
        <div class="col-12 mb-5">
            <h1>{{$mail->eap_user->username}} - {{$mail->subject}} - {{$mail->category}}
                - {{optional($mail->eap_user->company)->name}}
                - {{$mail->date}}</h1>
            <a href="{{route('operator.eap-online.mails.list', ['page' => $page])}}">{{__('common.back-to-list')}}</a>
        </div>
        <div class="col-12">
            @foreach($mail->eap_messages->sortBy('created_at') as $message)
                <div class="list-elem d-flex flex-column"
                    ...
                    <p>{{__('common.from')}}: {{$message->sender()->username}}</p>
                    <p style="font-family: CalibriI; font-weight: normal;">{!! $message->message !!}</p>
                </div>
            @endforeach
            @if(!isset($mail->deleted_at))
                <div class="list-elem">
                    <form action="{{route('operator.eap-online.mails.reply', ['id' => $mail->id])}}" method="post"
